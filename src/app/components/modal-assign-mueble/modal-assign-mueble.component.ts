...
@Component({
  selector: 'app-modal-assign-mueble',
  templateUrl: './modal-assign-mueble.component.html',
  styleUrls: ['./modal-assign-mueble.component.scss'],
})
export class ModalAssignMuebleComponent implements OnInit {
  @Input() mobiliario: any = null;
  ...
  getFurnitureId() {
    this.bitacoraService
      .getFurnitureId(this.mueblesId)
      .subscribe((mobiliario) => {
        this.furniture = mobiliario;
        this.userDeviceOwner(mobiliario.userId);
      });
  }
  ...
  changeOwner(idDevice: string) {
    this.bitacoraService
      .changeFurnitureOwner(this.selectedUser.userId, idDevice)
      .subscribe(
        (res) => {
          this.alert(
            'Cambiar propietario del mueble',
        ...
        (err) => {
          console.log(
            `id del usuario:${this.selectedUser.userId} \nid del mueble: ${idDevice}`
          );
          console.log(err);
          this.alert(
            'A ocurrido un problema inesperado',
            'No hemos podido cambiar el mueble de usuario. Intentarlo más tarde',
            [
              {
                text: 'Aceptar',
                role: 'confirm',
